...
function DroneController(name) {
	...
	_.forEach(this.controls, (def,name) => {
		this.robot.commands[name + 'Up'] = () => { moveStick( this.controls[name], this.controls[name].position + this.controls[name].increment ) }
		this.robot.commands[name + 'Down'] = () => { moveStick( this.controls[name], this.controls[name].position - this.controls[name].increment ) }
		this.robot.commands[name + 'Origin'] = () => { moveStick( this.controls[name], this.controls[name].origin ) }
	})
